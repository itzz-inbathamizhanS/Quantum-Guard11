...
`;
if (sidebarStart !== -1) html = html.substring(0, sidebarStart) + newSidebar + html.substring(sidebarEnd);

// 3. Replace Main Content Area
const mainStart = html.indexOf('<div class="flex-1 px-6');
const mainEnd = html.indexOf('</div>\n<!-- On This Page', mainStart) + 6;
const newMain = `
<div class="flex-1 px-6 md:px-12 lg:px-20 py-12 max-w-4xl flex flex-col">
    ...
    <article id="docs-content" class="prose prose-invert max-w-none space-y-6 markdown-body">
...
`;
if (mainStart !== -1) html = html.substring(0, mainStart) + newMain + html.substring(mainEnd);
...
const newToc = `
<aside class="hidden lg:block w-64 p-12 border-l border-outline-variant/10 sticky top-20 h-[calc(100vh-5rem)] overflow-y-auto no-scrollbar">
...
`;
if (tocStart !== -1) html = html.substring(0, tocStart) + newToc + html.substring(tocEnd);
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function loadMarkdown(filename, el) {
            ...
            try {
                // IMPORTANT: Use the exact structure that static files are hosted at.
                // Assuming docs are accessible via /docs/filename on the server
